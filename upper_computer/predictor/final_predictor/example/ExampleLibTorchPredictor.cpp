/**
 * @file ExampleLibTorchPredictor.cpp
...
/**
 * @class ExampleLibTorchPredictor::Impl
...
// ExampleLibTorchPredictor实现
ExampleLibTorchPredictor::ExampleLibTorchPredictor(const std::string& model_path,
                                     const std::string& model_info_path,
                                     const std::string& device)
...
ExampleLibTorchPredictor::~ExampleLibTorchPredictor() = default;

std::map<std::string, float> ExampleLibTorchPredictor::predict(const std::vector<float>& spectrum)
{
    return pimpl_->predict(spectrum);
}

std::map<std::string, std::vector<float>> ExampleLibTorchPredictor::predictBatch(const std::vector<std::vector<float>>& spectra)
{
    return pimpl_->predictBatch(spectra);
}

bool ExampleLibTorchPredictor::isModelLoaded() const
{
    return pimpl_->isModelLoaded();
}

int ExampleLibTorchPredictor::getInputSize() const
{
    return pimpl_->getInputSize();
}

int ExampleLibTorchPredictor::getOutputSize() const
{
    return pimpl_->getOutputSize();
}

std::vector<std::string> ExampleLibTorchPredictor::getPropertyLabels() const
{
    return pimpl_->getPropertyLabels();
}

std::vector<std::string> ExampleLibTorchPredictor::getWavelengthLabels() const
{
    return pimpl_->getWavelengthLabels();
}

void ExampleLibTorchPredictor::setLogCallback(std::function<void(const std::string&)> log_callback)
{
