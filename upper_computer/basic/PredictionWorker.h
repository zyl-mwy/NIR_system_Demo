...
public:
    ...
    // === 预测器管理接口 ===
    /**
     * @brief 设置Example光谱预测器
     * @param predictor Example光谱预测器指针
     * @details 设置用于执行预测的Example预测器实例
     *          预测器必须在主线程中创建，但可以在工作线程中使用
     */
    void setPredictor(ExampleSpectrumPredictor* predictor);
    
    /**
     * @brief 设置SVR光谱预测器
     * @param predictor SVR光谱预测器指针
     * @details 设置用于执行预测的SVR预测器实例
     *          预测器必须在主线程中创建，但可以在工作线程中使用
     */
    void setSVRPredictor(SVRLibTorchPredictor* predictor);
...
public slots:
     ...
     * @brief 执行SVR光谱预测任务
     * @param spectrum 输入光谱数据向量
     * @details 在后台线程中执行SVR光谱预测，避免阻塞主UI线程
     *          预测完成后通过predictionCompleted信号返回结果
     *          如果预测失败，通过predictionError信号返回错误信息
     */
    void performSVRPrediction(const QVector<double>& spectrum);
...
private:
    // === 私有成员变量 ===
    /**
     * @brief Example光谱预测器指针
     * @details 用于执行实际预测任务的Example预测器实例
     *          由主线程设置，在工作线程中使用
     */
    ExampleSpectrumPredictor* predictor_;
    
    /**
     * @brief SVR光谱预测器指针
     * @details 用于执行实际预测任务的SVR预测器实例
     *          由主线程设置，在工作线程中使用
     */
    SVRLibTorchPredictor* svrPredictor_;
};
