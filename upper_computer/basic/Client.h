...
// 光谱预测模块
#include "ExampleSpectrumPredictor.h"      // 光谱预测器
#include "SVRLibTorchPredictor.h"          // SVR预测器
...
private slots:
    ...
    /**
     * @brief 网络连接成功事件
     * @details 处理网络管理器发出的连接成功信号
     */
    void onNetworkConnected();
    
    /**
     * @brief 网络连接断开事件
     * @details 处理网络管理器发出的连接断开信号
     */
     ...
     * @details 处理网络管理器发出的JSON数据接收信号
     */
    void onJsonDataReceived(const QJsonObject &jsonData);
    
    /**
     * @brief 网络连接错误事件
     * @param error 错误类型
     * @param errorString 错误描述
     * @details 处理网络管理器发出的连接错误信号
     */
    void onNetworkError(QAbstractSocket::SocketError error, const QString &errorString);
    ...
    void updateDeviceStatus(const QJsonObject &deviceData);
    
    /**
     * @brief 更新上位机状态显示
     * @details 更新系统运行时间、光谱采集速率、连接状态等
     */
    void updateHostStatusDisplay();
...
private:
     ...
     */
    void initSVRSpectrumPredictor();
    
    /**
     * @brief 切换预测器类型
     * @param predictorType 预测器类型 ("example" 或 "svr")
     */
    void switchPredictorType(const QString &predictorType);
...
private:
    ...
    // === 系统监控模块 ===
    SystemMonitor *systemMonitor;  // 系统监控模块
    
    // === 网络通信模块 ===
    NetworkManager *networkManager;  // 网络通信模块
    ...
    QJsonObject lastDeviceStatus;   // 最新设备状态数据
    ...
    QString currentPredictorType;          // 当前预测器类型 ("example" 或 "svr")
